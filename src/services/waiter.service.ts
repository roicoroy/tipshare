...
export class WaiterService {
        ...
        cn.executeSql(`SELECT CRITERIA.CriteriaID
          ...
          .then(data => {
            ...
            if(data.rows.length > 0) {
              let temp;
              for(let i = 0; i < data.rows.length; i++) {
                temp = { name: data.rows.item(i).Name
  ...
  public removeCriteria(criteria: Criteria, waiter: Waiter) {
    return Observable.create(observer => {
      this.db.connectDb().subscribe(cn => {
        cn.executeSql(`DELETE FROM WAITER_CRITERIA 
          WHERE WaiterID = ? AND CriteriaID = ?;`,
           [criteria.criteriaId, waiter.waiterId])
          .then(() => {
            observer.next(criteria);
            observer.complete();
